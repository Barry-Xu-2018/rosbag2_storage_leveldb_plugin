...
#ifndef LEVELDB_PLUGIN_TEST_FIXTURE_HPP_
#define LEVELDB_PLUGIN_TEST_FIXTURE_HPP_
...
class LeveldbPluginTestFixture : public TemporaryDirectoryFixture
{
public:
  LeveldbPluginTestFixture()
  {
      ...
      [](rcutils_uint8_array_t * msg) {
        ...
        if (error != RCUTILS_RET_OK) {
          RCUTILS_LOG_ERROR_NAMED(
            "rosbag2_storage_leveldb_plugin", "Leaking memory %i", error);
        }
      });
  ...
  void write_messages_to_leveldb(
...
protected:
  ...
  {
    // This function also writes the final null character, which is absent in the CDR format.
  ...
  const std::string plugin_id_ = "leveldb";
};

#endif  // LEVELDB_PLUGIN_TEST_FIXTURE_HPP_
